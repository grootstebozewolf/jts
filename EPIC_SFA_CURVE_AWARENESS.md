...
- Public `DiscreteHausdorffDistance` (D-HF) — still sees chords in general; TestBuilder laser only. Closed-form exception: the two locked pairs above (witness √949/6 − 7/6, APEX). A single-member `MultiSurface` of one disc is the same two-disc pair (unwrap), not a third pair. `fail()` kept; message APEX, not `h=3`. Fréchet still open. D-HF honesty lives on draft [#45](https://github.com/grootstebozewolf/jts/pull/45).
...
- The class is the **full-TAG red list**, excluded from default Surefire. It is **not** the live scoreboard. Do **not** delete a method because a closed-form subset landed (OV-P1 kits, R-PR disc cells, disc area, TestBuilder Hausdorff, …). Delete only when the *full* TAG ships.
...
| **D-HF** | Two pairs only | Public `DiscreteHausdorffDistance` still sees chords in general. Closed-form exception (owned via `getGeometryType()`): (1) single-arc `CircularString` → single-segment `LineString`, (2) two circular discs. A single-member `MultiSurface` of one disc is the same two-disc pair (`circularDisc` unwrap), not a third pair. Witness √949/6 − 7/6 (APEX). TestBuilder laser only. Fréchet still open. No Linearizable-on-construct. Do not claim public Hausdorff is generally closed-form. Honesty: draft [#45](https://github.com/grootstebozewolf/jts/pull/45). |
...
Phase 1 (Foundations)
   ...
   ├──> Phase 3 (Measurement)           CurveExact helpers; D-HF half-red
                                        (public DHD still sees chords; Fréchet open)
