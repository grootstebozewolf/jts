...
| `feat:` | Public `DiscreteHausdorffDistance` and `DiscreteFrechetDistance` certified closed forms (two discs, arc→segment; Fréchet uses the Hausdorff-equal forms). TestBuilder `CurveExactFns` Hausdorff laser remains. | **D-HF partial**. Keep the spec method. |
...
- Full D-HF TAG (certified closed-form pairs landed; keep the spec method)
...
- The class is the **full-TAG red list**, excluded from default Surefire. It is **not** the live scoreboard. Do **not** delete a method because a closed-form subset landed (OV-P1 kits, R-PR disc cells, disc area, public Hausdorff / Fréchet pairs, …). Delete only when the *full* TAG ships.
...
| **D-HF** | Yes (landed) | Public `DiscreteHausdorffDistance` / `DiscreteFrechetDistance` certified closed forms (two discs, arc→segment; Fréchet uses Hausdorff-equal forms). Full TAG still partial. |
...
- **D-HF** — `DiscreteHausdorffDistance` / `DiscreteFrechetDistance` parameterise by arc length. **Partial:** public classes have certified closed forms (two discs, arc→segment; Fréchet uses Hausdorff-equal forms). Keep the spec method.
...
Phase 1 (Foundations)
   ...
   ├──> Phase 3 (Measurement)           CurveExact helpers; D-HF partial
                                        (public Hausdorff / Fréchet pairs)
