...
\texttt{CLOTHOID} / \texttt{BEZIER} / \texttt{ELLIPSE} / \texttt{NURBS}
are preview HOLD keywords (not SIGNED I/O, not the curve SoT).
Clothoid may also appear as a non-leading \texttt{COMPOUNDCURVE}
member. B\'ezier is a named fallback, not type 19.

WKB type codes 8--12 are the signed I/O set in core
\texttt{WKBReader} and \texttt{WKBConstants} (ISO/IEC 13249-3).
Preview 18--21 is HOLD: not SIGNED I/O. Construction still
requires a curve-capable factory; otherwise the reader throws.
\texttt{CurveWKBWriter} emits 8--12 and the preview HOLD layouts. A
bare \texttt{WKBWriter} still walks the parent \texttt{instanceof}
ladder for 8--12 and \textbf{refuses} to flatten preview 18--21.
ISO flavour adds $+1000/+2000/+3000$ for Z/M/ZM. EWKB default is
not ISO.

\textbf{Option B.} Public \texttt{SegmentString} names each span as
\texttt{LINEARIZED} / \texttt{ARC} / \texttt{CERTIFIED}. Default is
linearized. Exact consumers ask \texttt{mayCollapseToChord}. Indexes
may lie under \texttt{PrecisionModel} without renaming the kind.

\textbf{No silent densify.} \texttt{CurveLinearizationStrategy}
defaults to \texttt{LINEARIZED} and warns; \texttt{PRESERVE} keeps
type. TestBuilder Edit menu toggles the default; the Log tab shows
warn-sink lines.
...
signed I/O 8--12. Preview 18--21 is HOLD, not SIGNED I/O.

\begin{lstlisting}
// Read signed I/O 8-12 (ISO/IEC 13249-3):
Geometry g = new WKBReader(new CurveGeometryFactory()).read(bytes);
Geometry g2 = new CurveWKBReader().read(bytes);

// Write signed I/O 8-12:
byte[] wkb = new CurveWKBWriter().write(g);

// Bare writer: 8-12 may emit 2/3; preview 18-21 refuse flatten
