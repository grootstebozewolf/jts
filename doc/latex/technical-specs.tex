...
CircularString members of a CurvePolygon. Any other curve uses the
linear hull of \texttt{toLinear} --- a fallback, not a fail.
...
\begin{enumerate}
  \item A single-arc CircularString toward a single-segment
        LineString, apex $\sqrt{949}/6 - 7/6 \approx 3.967641$ for
        \texttt{CIRCULARSTRING (0 0, 2 3, 10 0)} vs
        \texttt{LINESTRING (0 0, 10 0)}.
  \item Two circular discs (full-circle CurvePolygon). A
        MultiSurface unwrap of one disc is the same disc pair.
\end{enumerate}
