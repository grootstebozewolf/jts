...
public class WKTCurvePolygonTest extends GeometryTestCase {
  ...
  public void testReadEmpty() throws Exception {
    Geometry g = new WKTReader().read("CURVEPOLYGON EMPTY");
    assertEquals(TYPENAME_CURVEPOLYGON, g.getGeometryType());
    assertTrue(g.isEmpty());
  }

  public void testReadEmptyZM() throws Exception {
    Geometry g = new WKTReader().read("CURVEPOLYGON ZM EMPTY");
    assertEquals(TYPENAME_CURVEPOLYGON, g.getGeometryType());
    assertTrue(g.isEmpty());
  }
  ...
  public void testRejectsUnclosedRing() throws Exception {
    // positive control
    assertNotNull(new WKTReader().read("CURVEPOLYGON((0 0, 1 0, 1 1, 0 1, 0 0))"));

    try {
      new WKTReader().read("CURVEPOLYGON((0 0, 1 0, 1 1, 0 1))");
