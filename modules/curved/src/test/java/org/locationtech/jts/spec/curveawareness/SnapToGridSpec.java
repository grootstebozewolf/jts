 ...
 *
 * <p>Run on demand:
 * {@code mvn -pl modules/curved test -Dtest=SnapToGridSpec}.
 */
public class SnapToGridSpec extends GeometryTestCase {
   ...
   * {@code CurvedPrecisionReducer.isGridFriendly(CircularString, PrecisionModel)}
  ...
  public void test_PRC_SN_isGridFriendlyHelperShape() throws Exception {
    ...
    // The implementation PR adds CurvedPrecisionReducer.isGridFriendly;
    // today the helper does not exist. Document via reflection so the
    // spec class compiles against the current jts-curved.
   ...
   * {@code CurvedPrecisionReducer.isGridFriendly(...)} static helper.
  ...
  private static boolean invokeIsGridFriendly(CircularString cs, PrecisionModel pm) {
    try {
      Class<?> cls = Class.forName(
          "org.locationtech.jts.precision.curved.CurvedPrecisionReducer");
