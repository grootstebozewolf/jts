...
public class CurvedBufferTest extends TestCase {
  ...
  private static CircularString stadium() {  // caps r2 at (+/-3,0), smooth convex
  ...
  public void testBoundaryParallelDistance() {
    ...
    int n = out.size();
    for (int i = 0; 2 * i + 2 < n; i++) {            // sample each output arc piece
  ...
  /** Curved buffer area matches core BufferOp on the densified source (general convex d>0). */
  public void testDensifiedCrossCheck() {
    ...
    for (CircularString s : shapes) {
      for (int it = 0; it < 12; it++) {
        double d = 0.3 + rnd.nextDouble() * 3.0;     // outward
  ...
  public void testReflexAndInwardCornerUnsupported() {
    // reflex (non-convex) chord ring, d>0
    CircularString reflex = cs(new double[][]{ {0,0},{5,0},{10,0}, {7.5,2.5},{5,5}, {2.5,2.5},{0,0} }); // dent at (5,5)? build a clear reflex
    ...
    // cornered ring eroded (d<0) -> degenerate
