...
final class ArcStringSimplicity {
  ...
  private static double[] circle(double[] p) {
    ...
    // edge-length-relative collinearity test: a chord built from a computed
    // midpoint has a ~1e-15 determinant crumb and must read as a chord, not a
    // spurious giant-radius arc (consistent with ArcRelate.circle).
