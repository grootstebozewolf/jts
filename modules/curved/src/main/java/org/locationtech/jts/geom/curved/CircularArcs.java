...
final class CircularArcs {
  ...
  static double[][] intersectArc(double sax, double say, double max, double may, double eax, double eay,
    ...
    final double EPS = 1e-9;
    // (near-)coincident centres: the radical line is degenerate, so isolated
    // points are undefined (two arcs of one circle differ only by ~1e-15 float in
    // their independently-fitted centres). Scale-relative guard avoids dividing by
    // a near-zero dd and returning a spurious far-off point.
    if (dd <= EPS * Math.max(1.0, Math.max(rA, rB))) return new double[0][];
