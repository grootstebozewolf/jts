 ...
 *
 * <p>Option A (F-CP): legacy {@code getExteriorRing()} and
 * {@code getInteriorRingN(i)} return {@link LinearRing} views obtained from
 * the control-point polyline of the structural ring (phase-1 linear view).
 * {@code toLinear(0.0)} currently returns the raw control points with no arc
 * tessellation; the tolerance parameter is accepted for {@link Linearizable}
 * compatibility but is a no-op in phase 1. Curve-aware code uses
...
public class CurvePolygon extends Polygon implements Linearizable {
  ...
  /**
   * Structural constructor: accepts shell and holes as any LineString
