...
2. Provide a `CurveGeometryReducer.isGridFriendly(CircularString,
...
3. **Implementation PR**:
   - Static helper
     `CurvePrecisionReducer.isGridFriendly(CircularString, PrecisionModel)`
   ...
   - Override `Geometry.reverse` / `apply` / the precision-reduction
     ...
     `CurvePrecisionReducer.reduce(curveGeom, pm)`.
