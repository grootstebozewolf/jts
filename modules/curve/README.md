...
> (`dev/check-no-curved.sh`) rejects the legacy `Curved` stem in new code.
> See `RENAME_CONTRACT.md` at the repo root for the full rule and allowlist.

## What it adds

| Geometry type      | Java class                                              | Extends                         |
|--------------------|---------------------------------------------------------|---------------------------------|
| `CircularString`   | `org.locationtech.jts.geom.curve.CircularString`       | `LineString`                    |
| `CompoundCurve`    | `org.locationtech.jts.geom.curve.CompoundCurve`        | `LineString`                    |
| `CurvePolygon`     | `org.locationtech.jts.geom.curve.CurvePolygon`         | `Polygon`                       |
| `MultiCurve`       | `org.locationtech.jts.geom.curve.MultiCurve`           | `MultiLineString`               |
| `MultiSurface`     | `org.locationtech.jts.geom.curve.MultiSurface`         | `MultiPolygon`                  |
| `Triangle`         | `org.locationtech.jts.geom.curve.Triangle`             | `Polygon`                       |
| `PolyhedralSurface`| `org.locationtech.jts.geom.curve.PolyhedralSurface`    | `MultiPolygon`                  |
| `Tin`              | `org.locationtech.jts.geom.curve.Tin`                  | `PolyhedralSurface`             |

Plus:

- `CurveGeometryFactory` — extends `GeometryFactory`, adds `createCircularString(...)`, `createTriangle(...)`, etc.
- `CurveWKTReader` — extends `WKTReader`, recognises the eight new keywords via the core `readOtherGeometryText` extension hook.
- `CurveWKTWriter` — extends `WKTWriter`. Phase-1 marker: the core writer already emits subclass keywords via `Geometry.getGeometryType().toUpperCase()`.
...
core; the geometry type lives in `org.locationtech.jts.geom.curve`.
...
CurveGeometryFactory factory = new CurveGeometryFactory();
CurveWKTReader reader = new CurveWKTReader(factory);
...
opt in by instantiating `CurveWKTReader`.
...
<dependency>
  ...
  <artifactId>jts-curve</artifactId>
...
explicitly instantiate `CurveWKTReader` / `CurveWKTWriter` /
`CurveGeometryFactory` when they want curve support. This keeps the
