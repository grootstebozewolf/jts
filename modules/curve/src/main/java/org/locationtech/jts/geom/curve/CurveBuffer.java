 ...
 * </ul>
 */
public final class CurveBuffer {
  ...
  public static Geometry buffer(Geometry geom, double d) {
    CurveGeometryFactory gf = factoryOf(geom);
  ...
  private static CurveGeometryFactory factoryOf(Geometry g) {
    return (g.getFactory() instanceof CurveGeometryFactory)
        ? (CurveGeometryFactory) g.getFactory() : new CurveGeometryFactory();
  }
  ...
  private static CoordinateSequence ringSeq(Geometry g) {
    ...
    throw new IllegalArgumentException("CurveBuffer requires a CurvePolygon or closed CircularString ring");
  }
