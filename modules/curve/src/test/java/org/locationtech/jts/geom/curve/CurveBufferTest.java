 ...
 */
public class CurveBufferTest extends TestCase {

  public static void main(String[] args) { TestRunner.run(CurveBufferTest.class); }
  public CurveBufferTest(String name) { super(name); }

  private static final CurveGeometryFactory GF = new CurveGeometryFactory();
  ...
  /**
   * Pins {@link CurveBuffer} to the exact buffer-region areas certified by the
  ...
  public void testOraclePinnedAreas() throws Exception {
    java.io.InputStream in = getClass().getResourceAsStream(
        "/org/locationtech/jts/geom/curve/rocqref/curve_buffer_region_vectors.txt");
    ...
    while ((line = r.readLine()) != null) {
      ...
      assertArcArea(label + " d=" + d, CurveBuffer.buffer(cp(shapeByLabel(label)), d), expected);
      checked++;
    }
    r.close();
  ...
  public void testAnalyticCircle() {
    for (double d : new double[]{ 0.5, 1, 2, 3 }) {
      Geometry b = CurveBuffer.buffer(cp(disk(0,0,5)), d);
      assertArcArea("circle d="+d, b, Math.PI * (5+d) * (5+d));
    }
  }

  /** CircularString.buffer(d) delegates to CurveBuffer, returns CurvePolygon. */
  ...
  public void testNegativeBufferCollapsesEmpty() {
    assertTrue("d=-5 -> EMPTY", CurveBuffer.buffer(cp(disk(0,0,5)), -5.0).isEmpty());
    assertTrue("d=-7 -> EMPTY", CurveBuffer.buffer(cp(disk(0,0,5)), -7.0).isEmpty());
    assertFalse("d=-4 -> non-empty", CurveBuffer.buffer(cp(disk(0,0,5)), -4.0).isEmpty());
  }
  ...
  public void testBoundaryParallelDistance() {
    ...
    Geometry buf = CurveBuffer.buffer(cp(src), d);
  ...
  public void testDensifiedCrossCheck() {
    ...
    for (CircularString s : shapes) {
      for (int it = 0; it < 12; it++) {
        ...
        Geometry buf = CurveBuffer.buffer(cp(s), d);
  ...
  public void testReflexAndInwardCornerUnsupported() {
    ...
    try { CurveBuffer.buffer(cp(arrow), 1.0); fail("reflex should be unsupported"); }
    catch (UnsupportedOperationException ok) { }
    try { CurveBuffer.buffer(cp(square10()), -1.0); fail("inward cornered should be unsupported"); }
    catch (UnsupportedOperationException ok) { }
  }
