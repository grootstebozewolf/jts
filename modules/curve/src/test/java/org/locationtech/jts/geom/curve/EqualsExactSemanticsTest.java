...
public class EqualsExactSemanticsTest extends GeometryTestCase {
  ...
  public void testWithMemberReplacedUsesHonestFactory() {
    CurveGeometryFactory f = cgf();
    LineString line = f.createLineString(new Coordinate[] {
        new Coordinate(0, 0), new Coordinate(10, 0)
    });
    ClothoidSegment cs = new ClothoidSegment(
        new Coordinate(10, 0), 0.0, 0.0, 0.005, 40.0, f);
    CompoundCurve cc = f.createCompoundCurve(new LineString[] { line, cs });
    ClothoidSegment replacement = new ClothoidSegment(
        new Coordinate(10, 0), 0.0, 0.0, 0.006, 40.0, f);
    CompoundCurve next = cc.withMemberReplaced(1, replacement);
    assertEquals(2, next.getNumMembers());
    assertTrue(next.getMemberN(1) instanceof ClothoidSegment);
    assertEquals(0.006, ((ClothoidSegment) next.getMemberN(1)).getEndKappa(), 0.0);
    assertFalse(cc.equalsExact(next));
  }
