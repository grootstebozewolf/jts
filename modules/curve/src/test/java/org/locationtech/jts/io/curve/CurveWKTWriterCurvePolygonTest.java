...
/**
 * FCP-WKT: {@link CurveWKTWriter} must emit the ring tag inside a
 ...
 */
public class CurveWKTWriterCurvePolygonTest extends GeometryTestCase {

  private static final String ARC_SHELL =
      "CURVEPOLYGON (CIRCULARSTRING (0 0, 4 0, 4 4, 0 4, 0 0))";

  public static void main(String[] args) {
    TestRunner.run(CurveWKTWriterCurvePolygonTest.class);
  }

  public CurveWKTWriterCurvePolygonTest(String name) { super(name); }

  private static String write(String wkt) throws Exception {
    return new CurveWKTWriter().write(new CurveWKTReader().read(wkt));
  }
  ...
  public void testRoundTripPreservesArcShell() throws Exception {
    String emitted = write(ARC_SHELL);
    CurvePolygon back = (CurvePolygon) new CurveWKTReader().read(emitted);
  ...
  public void testEmptyUnchanged() throws Exception {
    Geometry g = new CurveWKTReader().read("CURVEPOLYGON EMPTY");
