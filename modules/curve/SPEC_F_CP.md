...
Today `CurvePolygon` extends `Polygon` and the `CurveWKTReader` collapses
...
default tolerance for the linearised view becomes a `CurveGeometryFactory`-
...
3. **Implementation PR** (separate) starts from there:
   - Update `CurvePolygon` (`CurveGeometryFactory` constructors,
     `copyInternal`, `toLinear`).
   - Update `CurveWKTReader.readCurvePolygonText` to build a
     structural CurvePolygon.
   - Update `CurveWKTWriter` to emit COMPOUNDCURVE / CIRCULARSTRING
